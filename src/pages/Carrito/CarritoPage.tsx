...
function CarritoPage() {
  ...
  const { values, errors, handleChange, handleSubmit } = useForm<FormData>({
    ...
    validations: {
      ...
      direccion: (value) =>
        value.trim() === "" ? "La dirección es obligatoria." : "",
  ...
  const validateStock = async () => {
    try {
      ...
      for (const item of cart) {
        const productFromApi: Product | undefined = apiProducts.find(
          (p: any) => p.id === item.id,
        );

        if (productFromApi && item.quantity > productFromApi.stock) {
          setModalMessage(
            `Lo sentimos, el producto "${item.title}" tiene un stock limitado. Solo hay ${productFromApi.stock} unidades disponibles.`,
          );
      ...
      setModalMessage(
        "Ocurrió un error al validar el stock. Intenta de nuevo.",
      );
          ...
          <div className={styles.checkoutForm}>
            ...
            <form onSubmit={handleSubmit} className={styles.Form}>
              ...
              <div className={styles.formGroup}>
                ...
                {errors.celular && (
                  <span className={styles.errorText}>{errors.celular}</span>
                )}
              </div>
